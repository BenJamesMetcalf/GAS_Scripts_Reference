...
printf "Sample\temm_Type\tT_Antigen_Type\tST\tgki\tgtr\tmurI\tmutS\trecP\txpt\tyqiL\tMisc_Resistance\tSurface-Secretory_Type\n" >> "$out_analysis"/TABLE_GAS_"$batch_name"_Typing_Results.txt
...
do
    ...
    cat $final_outDir/TEMP_GAS_Typing_Results.txt >> $final_result_Dir/SAMPL_GAS_"$batch_name"_Typing_Results.txt
    cat $final_outDir/TEMP_table_results.txt >> $final_result_Dir/TABLE_GAS_"$batch_name"_Typing_Results.txt
